# static-web-app — Enterprise

<!-- Badges -->
![Terraform](https://img.shields.io/badge/Terraform-%3E%3D1.9-844FBA?logo=terraform&logoColor=white)
![AVM](https://img.shields.io/badge/Azure%20Verified%20Modules-13%20modules-0078D4?logo=microsoftazure&logoColor=white)
![Resources](https://img.shields.io/badge/Terraform%20Resources-40-844FBA)
![Tier](https://img.shields.io/badge/Tier-premium-brightgreen)
...
| **AVM Modules** | 13 Azure Verified Modules |
| **Terraform Resources** | 40 resources deployed |
| **Region** | `westeurope` |

> **Reference Architecture**: [Serverless Web Application](https://learn.microsoft.com/azure/architecture/web-apps/serverless/architectures/web-app) — Microsoft Azure Architecture Center
...
<details open>
<summary><strong>What each resource does and how it is secured</strong> (13 modules, 40 resources)</summary>
...
| **Virtual Network** | Isolated network for all services — subnets separate tiers (app, data, security). | NSG per subnet, no public IPs by default, DDoS protection in premium |
| **nat-gateway** | nat-gateway | See .tf file header |
| **Network Security Group** | Firewall rules per subnet — controls inbound/outbound traffic. | Deny-all default, explicit allow rules only, logged to Log Analytics |
| **Static Web App** | Serverless hosting for SPA/JAMstack (React, Vue, Next.js) — global CDN, custom domains, auth. | Free TLS certificates, Azure AD/GitHub auth, API backend integration |
| **Storage Account** | Object storage for data, logs, and artifacts. | TLS 1.2 configured, encryption at rest (AES-256), blob soft-delete, network deny by default (MCSB DP-1) |
...
| **Backup Vault** | Automated backup and disaster recovery (premium only). | Cross-region replication, immutable vaults, retention policies |
| **Private Endpoint** | Private network connection to PaaS services — no public internet exposure. | DNS integration, NSG support, service-specific access (MCSB NS-3) |
| **Private Endpoint** | Private network connection to PaaS services — no public internet exposure. | DNS integration, NSG support, service-specific access (MCSB NS-3) |
...
> and get written approval from your CISO/DPO before any production deployment.
...
Values in `terraform.tfvars` correspond to your chosen tier (**premium**).
...
| Variable | Default | Effect |
|:---------|:--------|:-------|
| `enable_storage_lifecycle` | `true` | Auto-tier blobs: Hot → Cool after N days |
| `storage_cool_after_days` | `30`/`60`/`90` (dev/uat/prod) | Days before moving to Cool tier (~50% cheaper) |

**To disable:** set `enable_storage_lifecycle = false` in `terraform.tfvars`.
...
| virtual-network | `Azure/avm-res-network-virtualnetwork/azurerm` | `0.17.1` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-network-virtualnetwork/azurerm) |
| nat-gateway | `Azure/avm-res-network-natgateway/azurerm` | `~> 0.3` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-network-natgateway/azurerm) |
| network-security-group | `Azure/avm-res-network-networksecuritygroup/azurerm` | `0.5.1` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-network-networksecuritygroup/azurerm) |
| static-web-app | `Azure/avm-res-web-staticsite/azurerm` | `~> 0.4` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-web-staticsite/azurerm) |
...
| backup-vault | `Azure/avm-res-recoveryservices-vault/azurerm` | `0.3.2` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-recoveryservices-vault/azurerm) |
| private-endpoint-kv | `Azure/avm-res-network-privateendpoint/azurerm` | `0.2.0` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-network-privateendpoint/azurerm) |
| private-endpoint-storage | `Azure/avm-res-network-privateendpoint/azurerm` | `0.2.0` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-network-privateendpoint/azurerm) |
...
<p align="center">
<sub>Generated by <strong>AethronOps v2</strong> — 13 modules AVM, 40 Terraform resources, premium tier</sub>
</p>
...
| key-vault | Operations (secrets, keys) | 0.0254 EUR/10K ops | 0 — 0 EUR |
| virtual-network | VNet is free. Peering and data transfer are charged separ... | Free | 0 EUR |
| nat-gateway | NAT Gateway | N/A | N/A |
| network-security-group | NSG is free. | Free | 0 EUR |
| static-web-app | Static Web App Free tier. Standard at 7.81 EUR/month. | Free | 0 EUR |
| storage-account | Blob storage LRS | 0.0203 EUR/GB | 0 — 2 EUR |
| application-insights | Data ingestion (5 GB/month free) | 1.9489 EUR/GB | 0 — 10 EUR |
| backup-vault | Backup protected instance | N/A | N/A |
| private-dns-zone | Private DNS Zone | N/A | N/A |
| private-endpoint | Private Endpoint | N/A | N/A |
| private-dns-zone | Private DNS Zone | N/A | N/A |
| private-endpoint | Private Endpoint | N/A | N/A |
| **Total** | | | **0 — 43 EUR/month** |
...
> Free tier: 5 GB/month., Static Web App Free tier. Standard at 7.81 EUR/month., Free tier: 5 GB/month.
...
**Tier** : premium
**Briques** : 15 (12 impactant le SLA)
...
| `key-vault` | 99.99% | SLA Azure Key Vault |
| `virtual-network` | 99.99% | SLA Azure Virtual Network |
| `nat-gateway` | 99.99% | SLA Azure NAT Gateway (data path) |
| `network-security-group` | 99.99% | SLA Azure Virtual Network (NSG inclus) |
| `static-web-app` | 99.95% | SLA Azure Static Web Apps |
| `storage-account` | 99.99% | 99.9% LRS/ZRS/GRS, 99.99% RA-GRS/RA-GZRS (lecture secondaire) |
| `application-insights` | 99.90% | SLA Azure Monitor Application Insights |
| `backup-vault` | 99.90% | SLA Azure Backup |
| `private-dns-zone` | 100% (pas d'impact) | SLA Azure DNS — 100% de disponibilité garantie |
| `private-endpoint` | 99.99% | SLA Azure Private Link |
| `private-dns-zone` | 100% (pas d'impact) | SLA Azure DNS — 100% de disponibilité garantie |
| `private-endpoint` | 99.99% | SLA Azure Private Link |

## SLA Composite

**SLA composite** : **99.5708%**
**Classe de disponibilité** : two nines (99%)

**Formule** : 99.90% × 99.99% × 99.99% × 99.99% × 99.99% × 99.99% × 99.95% × 99.99% × 99.90% × 99.90% × 99.99% × 99.99% = 99.5708%

## Temps d'indisponibilité estimé

| Période | Downtime |
|---------|----------|
| Par mois | 185.43 minutes |
| Par an | 2257.64 minutes (37.63 heures) |
