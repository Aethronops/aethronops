...
resource "azurerm_management_lock" "rg_lock" {
  ...
  depends_on = [module.application_insights, module.backup_vault, module.key_vault, module.log_analytics, module.managed_identity, module.nat_gateway, module.network_security_group, module.static_web_app, module.storage_account, module.virtual_network]
}
...
resource "azurerm_management_lock" "kv_lock" {
  ...
  lock_level = "ReadOnly"
  notes      = "AethronOps governance — Key Vault ReadOnly lock."
}
...
resource "azurerm_management_lock" "backup_lock" {
  count      = var.enable_resource_locks ? 1 : 0
  name       = "lock-backup-${var.project_name}-${var.environment}"
  scope      = module.backup_vault.resource_id
  lock_level = "CanNotDelete"
  notes      = "AethronOps governance — Backup Vault CanNotDelete lock. Deletion = loss of backups (DORA Art.12, BR-1/BR-2)."
}
...
# 365-day legal retention for audit trail integrity
...
# Secrets expire after 240 days, notify 60 days before
...
# Policy: Deny Public IP creation (NS-2, premium only)
resource "azurerm_resource_group_policy_assignment" "deny_public_ip" {
  count = var.enable_resource_locks ? 1 : 0

  name                 = "pol-deny-pip-${var.environment}"
  resource_group_id    = module.resource_group.resource_id
...
# ──────────────────────────────────────────────────────────
# BACKUP HEALTH ALERTS — MCSB BR-3 (Monitor backups)
# Alerts on backup job failures for all protected resources.
# DORA Art.12: backup policies must be monitored continuously.
# ──────────────────────────────────────────────────────────
resource "azurerm_monitor_metric_alert" "backup_failure" {
  count = var.enable_resource_locks ? 1 : 0  # same lifecycle as governance

  name                = "alert-backup-failure-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.backup_vault.resource_id]
  description         = "Backup job failed — investigate immediately (MCSB BR-3, DORA Art.12)"
  severity            = 1  # Critical
  frequency           = "PT1H"
  window_size         = "PT6H"
  ...
  dynamic "action" {
    ...
    content {
      action_group_id = azurerm_monitor_action_group.finops[0].id
    }
  }
