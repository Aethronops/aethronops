...
locals {
  ...
  common_tags = merge(var.tags, {
    ...
    # ── Operations ──
    stack_type          = "storage-baseline"
    tier                = "premium"
    deployment_date     = "managed-by-ci"  # Set by CI/CD pipeline or manual deploy
  })
}

# Existing private DNS zone — brownfield mode (centralized Hub)
# Option 1: provide var.existing_private_dns_zone_kv_id directly (recommended)
# Option 2: enable var.brownfield_lookup + configure var.existing_private_dns_zone_kv_rg
data "azurerm_private_dns_zone" "private_dns_zone_kv" {
  count               = var.existing_private_dns_zone_kv_id == "" && var.brownfield_lookup ? 1 : 0
  name                = "privatelink.vaultcore.azure.net"
  resource_group_name = var.existing_private_dns_zone_kv_rg
}

locals {
  private_dns_zone_kv_id = var.existing_private_dns_zone_kv_id != "" ? var.existing_private_dns_zone_kv_id : try(data.azurerm_private_dns_zone.private_dns_zone_kv[0].id, "")
}

# Existing private DNS zone — brownfield mode (centralized Hub)
# Option 1: provide var.existing_private_dns_zone_storage_id directly (recommended)
# Option 2: enable var.brownfield_lookup + configure var.existing_private_dns_zone_storage_rg
data "azurerm_private_dns_zone" "private_dns_zone_storage" {
  count               = var.existing_private_dns_zone_storage_id == "" && var.brownfield_lookup ? 1 : 0
  name                = "privatelink.blob.core.windows.net"
  resource_group_name = var.existing_private_dns_zone_storage_rg
}

locals {
  private_dns_zone_storage_id = var.existing_private_dns_zone_storage_id != "" ? var.existing_private_dns_zone_storage_id : try(data.azurerm_private_dns_zone.private_dns_zone_storage[0].id, "")
}
