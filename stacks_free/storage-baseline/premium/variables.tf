...
# Override per environment to avoid CIDR overlap in hub-spoke.
# ──────────────────────────────────────────────────────────

variable "vnet_address_space" {
  description = "CIDR du VNet (ex: 10.0.0.0/16 pour hub, 10.1.0.0/16 pour spoke-dev)"
  type        = string
  default     = "10.0.0.0/16"
}

# ──────────────────────────────────────────────────────────
# HUB-SPOKE PEERING — Connect this spoke VNet to a hub VNet
# Leave hub_vnet_id empty to skip peering.
# ──────────────────────────────────────────────────────────

variable "hub_vnet_id" {
  description = "Resource ID of the hub VNet for peering (leave empty to skip). NOTE: peering modifies the existing hub network."
  type        = string
  default     = ""
  validation {
    condition     = var.hub_vnet_id == "" || can(regex("^/subscriptions/[a-f0-9-]+/resourceGroups/[^/]+/providers/Microsoft.Network/virtualNetworks/[^/]+$", var.hub_vnet_id))
    error_message = "hub_vnet_id must be a valid Azure Resource ID (/subscriptions/.../virtualNetworks/...) or empty."
  }
}

variable "hub_vnet_name" {
  description = "Hub VNet name (e.g. vnet-hub-prod-frc). Must match the VNet in hub_vnet_id."
  type        = string
  default     = ""
}

variable "hub_resource_group_name" {
  description = "Resource Group name containing the hub VNet. The service principal needs Network Contributor on it."
  type        = string
  default     = ""
}

# Cross-validation: all 3 hub variables must be all empty or all set.
# Prevents a partial peering that would leave the network in an inconsistent state.
locals {
  hub_peering_enabled = var.hub_vnet_id != ""
  _hub_validation = (var.hub_vnet_id == "" && var.hub_vnet_name == "" && var.hub_resource_group_name == "") || (var.hub_vnet_id != "" && var.hub_vnet_name != "" && var.hub_resource_group_name != "")
}

resource "terraform_data" "hub_peering_validation" {
  lifecycle {
    precondition {
      condition     = local._hub_validation
      error_message = "NETWORK ERROR: hub_vnet_id, hub_vnet_name and hub_resource_group_name must ALL be set or ALL be empty. A partial peering configuration is dangerous."
    }
  }
}
...
# Option 1 (recommended): provide existing_xxx_id directly
# Option 2: enable brownfield_lookup + configure name/rg for lookup
# ============================================================

variable "brownfield_lookup" {
  description = "Enable brownfield resource lookup by name (resources must already exist)"
  type        = bool
  default     = false
}

variable "existing_private_dns_zone_kv_id" {
  description = "Resource ID of existing private DNS zone (private-dns-zone-kv). Leave empty to use lookup."
  type        = string
  default     = ""
}

variable "existing_private_dns_zone_kv_rg" {
  description = "Resource Group of the private DNS zone (private-dns-zone-kv)"
  type        = string
  default     = "REPLACE_ME"
}

variable "existing_private_dns_zone_storage_id" {
  description = "Resource ID of existing private DNS zone (private-dns-zone-storage). Leave empty to use lookup."
  type        = string
  default     = ""
}

variable "existing_private_dns_zone_storage_rg" {
  description = "Resource Group of the private DNS zone (private-dns-zone-storage)"
  type        = string
  default     = "REPLACE_ME"
}
