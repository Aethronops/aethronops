...
variable "governance_scope" {
  description = "Management Group ID for policy assignments (e.g. azurerm_management_group.mg.id)"
  type        = string
  default     = ""  # Set to management group resource ID
}

variable "required_tags" {
  description = "List of mandatory tag names for the require-tags policy"
  type        = list(string)
  default     = ["environment"]
}

variable "enable_sandbox_exemption" {
  description = "Enable policy exemption for sandbox Management Group"
  type        = bool
  default     = false
}
