...
locals {
  ...
  common_tags = merge(var.tags, {
    ...
    # ── Operations ──
    stack_type          = "governance-organization"
    tier                = "basic"
    deployment_date     = "managed-by-ci"  # Set by CI/CD pipeline or manual deploy
  })

  # Governance scope — auto-wired to the management group created in this stack
  governance_scope_id = var.governance_scope != "" ? var.governance_scope : azurerm_management_group.management_group.id
}
