...
variable "mongodb_autoscale_enabled" {
  description = "MongoDB vCore autoscale (compute scales with load). GA 2026. Free. NOTE: AVM module does not yet expose this — activate post-deploy via portal."
  type        = bool
  default     = true
}
...
# ──────────────────────────────────────────────────────────
# MONGODB — Azure DocumentDB (MongoDB vCore) configuration
# ──────────────────────────────────────────────────────────

variable "mongodb_compute_tier" {
  description = "Compute tier: Free (dev), M10/M20/M25 (burstable), M30+ (standard, HA capable)"
  type        = string
  default     = "M30"
}

variable "mongodb_storage_gb" {
  description = "Storage size in GB (32, 64, 128, 256, 512, 1024, 2048)"
  type        = number
  default     = 128
}
...
variable "mongodb_version" {
  ...
  default     = "8.0"
  validation {
    condition     = contains(["5.0", "6.0", "7.0", "8.0"], var.mongodb_version)
    error_message = "mongodb_version must be 5.0, 6.0, 7.0, or 8.0."
  }
}

variable "mongodb_ha_mode" {
  description = "High availability: Disabled, SameZone, ZoneRedundantPreferred. Requires M30+ tier."
  type        = string
  default     = "Disabled"
  validation {
    condition     = contains(["Disabled", "SameZone", "ZoneRedundantPreferred"], var.mongodb_ha_mode)
    error_message = "mongodb_ha_mode must be Disabled, SameZone, or ZoneRedundantPreferred."
  }
}

variable "mongodb_backup_policy" {
  description = "Backup policy: Periodic, Continuous7Days, Continuous30Days"
  type        = string
  default     = "Continuous7Days"
  validation {
    condition     = contains(["Periodic", "Continuous7Days", "Continuous30Days"], var.mongodb_backup_policy)
    error_message = "mongodb_backup_policy must be Periodic, Continuous7Days, or Continuous30Days."
  }
}

variable "mongodb_admin_login" {
  description = "MongoDB administrator login name (4-31 chars, starts with letter)"
  type        = string
  default     = "mongoAdmin"
}
...
# ──────────────────────────────────────────────────────────
# CONTAINER APP — Image and scaling
# ──────────────────────────────────────────────────────────
...
variable "container_cpu" {
  description = "Container CPU cores (0.25, 0.5, 1, 2, 4)"
  type        = number
  default     = 0.25
}

variable "container_memory" {
  description = "Container memory (0.5Gi, 1Gi, 2Gi, 4Gi, 8Gi)"
  type        = string
  default     = "0.5Gi"
}
