...
# CPU > 80%
resource "azurerm_monitor_metric_alert" "mongo_cpupercent" {
  count               = local.db_alerts_enabled
  name                = "alert-mongo-cpupercent-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mongo_cluster.mongo_cluster_id]
  description         = "CPU > 80%"
  severity            = 2
  frequency           = "PT5M"
  window_size         = "PT15M"
...
# Storage > 85%
resource "azurerm_monitor_metric_alert" "mongo_storagepercent" {
  count               = local.db_alerts_enabled
  name                = "alert-mongo-storagepercent-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mongo_cluster.mongo_cluster_id]
