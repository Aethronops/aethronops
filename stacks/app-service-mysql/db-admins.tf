...
# ──────────────────────────────────────────────────────────
# MySQL Entra admin (AVM module doesn't expose authentication
# input — pattern required for MySQL Flexible Server 2026).
# ──────────────────────────────────────────────────────────
resource "azurerm_mysql_flexible_server_active_directory_administrator" "mysql_aad" {
  count       = var.enable_db_passwordless ? 1 : 0
  server_id   = module.mysql_flexible.resource_id
  identity_id = module.managed_identity.resource_id
  login       = azuread_group.db_admins[0].display_name
