...
#   - MySQL Flexible Server (stop/start)
#
# How it works:
...
# Role: allow Automation to stop/start MySQL Flexible Servers
resource "azurerm_role_assignment" "finops_mysql_contributor" {
  count = var.enable_finops && var.enable_paas_schedule ? 1 : 0

  scope                = module.resource_group.resource_id
  role_definition_name = "Contributor" # No built-in narrow role for MySQL Flex stop/start
  principal_id         = azurerm_automation_account.finops[0].identity[0].principal_id
  principal_type       = "ServicePrincipal"
  lifecycle { ignore_changes = [principal_type] }
}
...
resource "azurerm_automation_runbook" "paas_schedule" {
  ...
  content = <<-POWERSHELL
    ...
    # ── MySQL Flexible Server: stop off-hours, start on-hours ──
    $mysqlServers = Get-AzMySqlFlexibleServer -ResourceGroupName $rgName -ErrorAction SilentlyContinue
    foreach ($mysql in $mysqlServers) {
      $tag = $mysql.Tag["Schedule"]
      if (-not $tag) { continue }
      try {
        $schedule = $tag | ConvertFrom-Json
        if ($activeDays -contains $dayName -and $currentTime -ge $schedule.start -and $currentTime -lt $schedule.stop) {
          if ($mysql.State -eq "Stopped") {
            Write-Output "Starting MySQL $($mysql.Name)"
            Start-AzMySqlFlexibleServer -ResourceGroupName $rgName -Name $mysql.Name -NoWait
          }
        } else {
          if ($mysql.State -eq "Ready") {
            Write-Output "Stopping MySQL $($mysql.Name)"
            Stop-AzMySqlFlexibleServer -ResourceGroupName $rgName -Name $mysql.Name -NoWait
          }
        }
      } catch {
        Write-Warning "Error processing MySQL $($mysql.Name): $_"
      }
    }
