...
# CPU > 80%
resource "azurerm_monitor_metric_alert" "mysql_cpu_percent" {
  count               = local.db_alerts_enabled
  name                = "alert-mysql-cpu-percent-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mysql_flexible.resource_id]
  description         = "CPU > 80%"
  severity            = 2
  frequency           = "PT5M"
  window_size         = "PT15M"
...
# Memory > 80%
resource "azurerm_monitor_metric_alert" "mysql_memory_percent" {
  count               = local.db_alerts_enabled
  name                = "alert-mysql-memory-percent-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mysql_flexible.resource_id]
...
# Storage > 85%
resource "azurerm_monitor_metric_alert" "mysql_storage_percent" {
  count               = local.db_alerts_enabled
  name                = "alert-mysql-storage-percent-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mysql_flexible.resource_id]
...
resource "azurerm_monitor_metric_alert" "mysql_active_connections" {
  count               = local.db_alerts_enabled
  name                = "alert-mysql-active-connections-${local.name_prefix}"
  resource_group_name = module.resource_group.name
  scopes              = [module.mysql_flexible.resource_id]
