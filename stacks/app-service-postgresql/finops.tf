...
# Role: allow Automation to stop/start PostgreSQL Flexible Servers
resource "azurerm_role_assignment" "finops_pg_contributor" {
  count = var.enable_finops && var.enable_paas_schedule ? 1 : 0

  scope                = module.resource_group.resource_id
  role_definition_name = "Contributor" # No built-in narrow role for PG Flex stop/start
  principal_id         = azurerm_automation_account.finops[0].identity[0].principal_id
  principal_type       = "ServicePrincipal"
  lifecycle { ignore_changes = [principal_type] }
}
...
resource "azurerm_automation_runbook" "paas_schedule" {
  ...
  content = <<-POWERSHELL
    ...
    # ── PostgreSQL Flexible Server: stop off-hours, start on-hours ──
    $pgServers = Get-AzPostgreSqlFlexibleServer -ResourceGroupName $rgName -ErrorAction SilentlyContinue
      ...
      try {
        ...
        if ($activeDays -contains $dayName -and $currentTime -ge $schedule.start -and $currentTime -lt $schedule.stop) {
          if ($pg.State -eq "Stopped") {
            Write-Output "Starting PostgreSQL $($pg.Name)"
            Start-AzPostgreSqlFlexibleServer -ResourceGroupName $rgName -Name $pg.Name -NoWait
          }
        } else {
          if ($pg.State -eq "Ready") {
            Write-Output "Stopping PostgreSQL $($pg.Name)"
            Stop-AzPostgreSqlFlexibleServer -ResourceGroupName $rgName -Name $pg.Name -NoWait
          }
        }
      } catch {
        Write-Warning "Error processing PostgreSQL $($pg.Name): $_"
      }
    }
