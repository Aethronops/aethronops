...
![Resources](https://img.shields.io/badge/Terraform%20Resources-33-844FBA)
...
> **Cost & Performance** — ~€15-30/month, deploys in 5-8 minutes, 33 Terraform resources.
...
```hcl
module "app_service_postgresql" {
  source  = "aethronops/app-service-postgresql/azurerm"
  version = "~> 3.0"
...
| | |
|---|---|
| **Pattern** | `app-service-postgresql` |
| **Tier** | dev (Dev) |
| **AVM Modules** | 8 Azure Verified Modules |
| **Terraform Resources** | 33 resources deployed |
...
<details open>
<summary><strong>What each resource does and how it is secured</strong> (8 modules, 33 resources)</summary>
...
| **App Service Plan** | Compute tier for App Service/Functions — defines CPU/RAM/scaling. | Function stacks use Linux Consumption Y1 (serverless, pay-per-execution). App Service stacks use B1/P1v3/P3v3 per tier. |
| **App Service** | Managed web application hosting — auto-scaling, deployment slots. | HTTPS only, TLS 1.3 enforced, managed identity, VNet integration in premium (MCSB DP-3/NS-3) |
| **PostgreSQL Flexible Server** | Managed relational database — HA, automated backups, scaling. | TLS 1.3 enforced, VNet integration in premium, automated backups (MCSB DP-1/DP-3) |
...
### PaaS Start/Stop Scheduling

| Variable | Default | Effect |
|:---------|:--------|:-------|
| `enable_paas_schedule` | `true` (dev/uat), `false` (prod) | Automated PaaS start/stop or scale-down |
| `schedule_timezone` | `"Europe/Paris"` | Shared timezone for all schedules |

**Covered services in this stack:**
- **App Service Plan** — scaled down to B1 off-hours, original SKU restored on-hours
- **PostgreSQL Flexible** — stopped off-hours, started on-hours

**How it works:** same tag pattern as VMs — tag each PaaS resource with `Schedule` JSON.
The Automation Account runbook runs hourly.

**To disable:** set `enable_paas_schedule = false` in `envs/<env>.tfvars`.
...
| postgresql-flexible | `Azure/avm-res-dbforpostgresql-flexibleserver/azurerm` | `0.2.0` | [Registry](https://registry.terraform.io/modules/Azure/avm-res-dbforpostgresql-flexibleserver/azurerm) |
...
<p align="center">
<sub>Generated by <strong>AethronOps v3</strong> — 8 modules AVM, 33 Terraform resources, dev tier</sub>
</p>
