...
variable "cosmosdb_autoscale_enabled" {
  description = "CosmosDB autoscale for containers (not account-level). Free. NOTE: configure per-container via autoscale_settings when creating SQL containers via app code or separate Terraform."
  type        = bool
  default     = true
}
