...
# ──────────────────────────────────────────────────────────
# Cosmos DB Data Plane RBAC — Entra group = Data Contributor
# Built-in role GUID 00000000-0000-0000-0000-000000000002 =
# "Cosmos DB Built-in Data Contributor" (full read/write).
# Combined with local_authentication_disabled = true on the
# Cosmos account, this enforces Entra-only access.
# ──────────────────────────────────────────────────────────
resource "azurerm_cosmosdb_sql_role_assignment" "cosmos_db_admins" {
  count               = var.enable_db_passwordless ? 1 : 0
  account_name        = module.cosmosdb_account.name
  resource_group_name = module.resource_group.name
  role_definition_id  = "${module.cosmosdb_account.resource_id}/sqlRoleDefinitions/00000000-0000-0000-0000-000000000002"
  principal_id        = azuread_group.db_admins[0].object_id
  scope               = module.cosmosdb_account.resource_id
}
