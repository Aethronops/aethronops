...
# SQL admin password is auto-generated via random_password.sql_admin
# and stored in Key Vault. No variable needed — zero hardcoded secrets.

variable "sql_aad_admin_login" {
  description = "Entra ID administrator login for SQL"
  type        = string
  default     = "sqlaadadmin@example.com"
}

variable "sql_aad_admin_object_id" {
  description = "Entra ID administrator object ID for SQL (null = deployer identity)"
  type        = string
  default     = null
}

variable "sql_maintenance_window" {
  description = "Azure public maintenance configuration for the SQL Database. 'SQL_Default' = system default (5pm-8am local time, blocks patches 8am-5pm). Non-default options per region: SQL_<Region>_DB_1 = weekday 10pm-6am Mon-Thu, SQL_<Region>_DB_2 = weekend 10pm-6am Fri-Sun. See: https://learn.microsoft.com/en-us/azure/azure-sql/database/maintenance-window"
  type        = string
  default     = "SQL_Default"
}

variable "sql_automatic_tuning_enabled" {
  description = "Azure SQL Automatic Tuning (AI auto-creates/drops indexes). Free. NOTE: activate post-deploy via Azure portal or SQL command — not yet available as Terraform resource (2026 limitation)."
  type        = bool
  default     = true
}
